...
char   *buffer_t_buffer(buffer_t *ciphertext)
{
    return ciphertext->array;
}
size_t buffer_t_len(buffer_t *ciphertext)
{
    return ciphertext->len;
}
int buffer_t_copy(buffer_t *ciphertext,const char *data,size_t capacity)
{
    if (ciphertext->capacity >= capacity){
        memcpy(ciphertext->array, data, capacity);
        ciphertext->len = capacity;
    }else {
        return -1;
    }
...
enc_ctx_t *enc_ctx_create()
{
    size_t size = sizeof(enc_ctx_t);
    
    return malloc(sizeof(enc_ctx_t));
}
int free_enc_ctx(enc_ctx_t * ctx)
{
...
#elif defined(USE_CRYPTO_MBEDTLS)
    mbedtls_cipher_context_t *evp         = (mbedtls_cipher_context_t *)&ctx->evp;
    if (evp != NULL){
        mbedtls_cipher_free(evp);
    }
#endif
...
int enc_init(const char *pass, const char *method,size_t len,size_t mlen)
{
    int m = TABLE;
    char password[len+1];
    ...
    char methodcopy[mlen+1];
    memset(methodcopy, 0, mlen+1);
    memcpy(methodcopy, method, mlen);
    
    if (method != NULL) {
        for (m = TABLE; m < CIPHER_NUM; m++)
            if (strcmp(methodcopy, supported_ciphers[m]) == 0) {
                break;
            }
        if (m >= CIPHER_NUM) {
    ...
    if (m == TABLE) {
        enc_table_init(password);
    } else {
        enc_key_init(m, password);
    }
    return m;
}
...
void sss_onetimeauth(buffer_t *buf, struct enc_ctx *ref,size_t len)
{
    ss_onetimeauth(buf, ref->evp.iv, len);
